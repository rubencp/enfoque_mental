...
RSpec.describe User, type: :model do
  describe '#next_message' do
    ...
    it 'responds with the first message of the conversation' do
      expect(user_conversation.next_message).to eq('¿Quién eres?')
    end

    it 'gets different message on 2nd time' do
      user_conversation.next_message
      user_conversation.increment_conversation
      expect(user_conversation.next_message).to eq('¿Cuál es tu propósito en esta vida?')
    end

    it 'responds with the first message after all messages' do
      7.times { user_conversation.next_message }
